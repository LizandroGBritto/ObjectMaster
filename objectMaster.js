...
const DivisibleBy3 = pokemon.map(p => p.id % 3 === 0); 
console.log(DivisibleBy3);

const fireTypePokemon = pokemon.filter(p => p.types.includes("fire"));
console.log(fireTypePokemon);

const dualTypePokemon = pokemon.filter(p => p.types.length > 1);
console.log(dualTypePokemon);

const pokemonNames = pokemon.map(p => p.name);
console.log(pokemonNames);

const highIdPokemonNames = pokemon.filter(p => p.id > 99).map(p => p.name);
console.log(highIdPokemonNames);

const poisonTypePokemonNames = pokemon.filter(p => p.types.length === 1 && p.types.includes("poison")).map(p => p.name);
